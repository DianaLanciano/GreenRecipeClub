...
public class ModelFirebase {
    ...
    public static void setUserAppData(final String email) {
        ...
        db.collection("userProfileData").document(email).get().addOnCompleteListener(task -> {
        ...
        if (imageUri != null) {
            ...
            uploadTask.continueWithTask(task -> {
                ...
                if (task.isSuccessful()) {
                    ...
                    data.put("info", "NA");
                    firebaseFirestore.collection("userProfileData").document(email).set(data).addOnSuccessListener(aVoid -> {
                        if (firebaseAuth.getCurrentUser() != null) {
                            firebaseAuth.signOut();
                        }
                    }).addOnFailureListener(e -> Toast.makeText(MyApplication.context, "Fails to create user and upload data: " + e.getMessage(), Toast.LENGTH_SHORT).show());
    ...
    public static void addRecipe(Recipe recipe, final Model.Listener<Boolean> listener) {
        FirebaseFirestore db = FirebaseFirestore.getInstance();
        db.collection(RECIPE_COLLECTION).document(recipe.getRecipeId()).set(toMap(recipe)).addOnCompleteListener(task -> {
            if (listener != null) {
    ...
    public static void updateUserProfile(String username, String profileImgUrl, final Model.Listener<Boolean> listener) {
        ...
        db.collection("userProfileData").document(User.getInstance().email).set(json).addOnCompleteListener(task -> {
            if (listener != null)
                listener.onComplete(task.isSuccessful());
        });
    }
    ...
    public static void getDeletedRecipesId(final Model.Listener<List<String>> listener) {
        FirebaseFirestore db = FirebaseFirestore.getInstance();
        db.collection("deleted").get().addOnCompleteListener(task -> {
            List<String> deletedRecipesIds = null;
            if (task.isSuccessful()) {
                deletedRecipesIds = new LinkedList<>();
                for (QueryDocumentSnapshot doc : task.getResult()) {
                    String deleted = (String) doc.getData().get("recipeId");
                    deletedRecipesIds.add(deleted);
                }
            }
            listener.onComplete(deletedRecipesIds);
        });
    }

    public static void deleteRecipe(Recipe recipe, final Model.Listener<Boolean> listener) {
        final FirebaseFirestore db = FirebaseFirestore.getInstance();
        db.collection(RECIPE_COLLECTION).document(recipe.getRecipeId()).delete().addOnCompleteListener(task -> {
            Map<String, Object> deleted = new HashMap<>();
            deleted.put("recipeId", recipe.getRecipeId());
            db.collection("deleted").document(recipe.getRecipeId()).set(deleted).addOnCompleteListener(task1 -> {
                if (listener != null) {
                    listener.onComplete(task1.isSuccessful());
                }
            });
        });
    }
